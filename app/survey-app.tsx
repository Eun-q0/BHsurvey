    ...
    <main className="mx-auto max-w-5xl px-4 py-7 sm:px-8 sm:py-10">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px] lg:items-start">
        ...
        <aside className="rounded-xl border border-[#d7dee8] bg-white p-5 lg:sticky lg:top-24">
          ...
          {ended ? <Button disabled className="mt-5 min-h-12 w-full">종료된 설문입니다</Button> : <Button asChild className="mt-5 min-h-12 w-full bg-[#13294b] hover:bg-[#1f3b68]"><a href={survey.naver_form_url} target="_blank" rel="noopener noreferrer">네이버 폼에서 참여하기 <ExternalLink /></a></Button>}
...
function RegisterView({ navigate, refresh }: { navigate: (view: View, id?: string) => void; refresh: () => void }) {
  ...
  const validUrl = (() => { try { new URL(draft.naver_form_url); return true; } catch { return false; } })();
  const naverWarning = draft.naver_form_url && validUrl && !new URL(draft.naver_form_url).hostname.endsWith("naver.com");
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-7 sm:px-8 sm:py-10"><button onClick={() => navigate("home")} className="mb-5 flex min-h-11 items-center gap-2 font-semibold text-[#526078]"><ArrowLeft /> 홈으로</button><div className="mb-7"><p className="text-sm font-semibold text-[#496584]">새 설문 등록</p><h1 className="mt-1 text-3xl font-bold tracking-[-0.04em] text-[#13294b]">네이버 폼을 학생들과 공유하세요</h1><p className="mt-2 leading-relaxed text-[#67758a]">설문 자체는 네이버 폼에서 만들고, 참여자가 이해할 수 있도록 조사 정보를 입력해주세요.</p></div>
      <form onSubmit={submit} className="space-y-7 rounded-xl border border-[#d7dee8] bg-white p-5 sm:p-8">
        ...
        <div className="grid gap-6 sm:grid-cols-2"><Field label="조사 대상" required><Input value={draft.target} onChange={(e) => set("target", e.target.value)} className="h-11" placeholder="예: 청주여고 전교생" /></Field><Field label="예상 소요 시간" required><Input value={draft.duration} onChange={(e) => set("duration", e.target.value)} className="h-11" placeholder="예: 약 3분" /></Field></div>
        ...
        <Field label="네이버 폼 URL" required hint="네이버 폼에서 공유 링크를 복사해 붙여넣어주세요."><Input type="url" value={draft.naver_form_url} onChange={(e) => set("naver_form_url", e.target.value)} className="h-11" placeholder="https://form.naver.com/..." />{draft.naver_form_url && !validUrl && <p className="flex items-center gap-1.5 text-sm text-[#b42318]"><AlertTriangle className="size-4" /> 올바른 URL 형식으로 입력해주세요.</p>}{naverWarning && <p className="flex items-center gap-1.5 text-sm text-[#a45a17]"><AlertTriangle className="size-4" /> 네이버 폼 링크인지 다시 확인해주세요. 저장은 가능해요.</p>}</Field>
